...
pktxt = open('walletdata.txt', 'r')
privatekeystringvar.set(pktxt.read())
pktxt.close()


publickeystringvar = StringVar()
privatekey = SigningKey.from_string(
    codecs.decode(pktxt.read(), 'hex'), curve=SECP256k1)
publickeystringvar.set(privatekey.to_string().hex())
