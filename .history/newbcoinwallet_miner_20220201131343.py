...
sendcoinsbutton = Button(window, text="send coins", width=30, command=sendcoins)
...
miningprogressbar = Progressbar(window, orient = HORIZONTAL,length = 200, mode = 'determinate')
miningprogressbar.grid(row=8, column=1)

startminingbutton = Button(window, text="start mining", width=30, command=minecoins)
...
privatekeystringvar.set("12345")

publickeystringvar = StringVar()
publickeystringvar.set("12344556657768990")

privatekeylabel = Label(window, text="private key \n hover to show key")
privatekeylabel.grid(row=7, column=3)


privatekeytext = Entry(window, textvariable=privatekeystringvar,state='readonly', foreground="white")
...
publickeytext = Entry(window, textvariable=publickeystringvar,state='readonly')
publickeytext.grid(row=10, column=3)

generatewalletbutton = Button(window, text="generate wallet", width=30, command=generatewallet)
