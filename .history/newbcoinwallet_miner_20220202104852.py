...
def getmybalance():
    if statusvar:
        balance = requests.request("GET", balanceurl+str(publickeystringvar))
        return str(balance)
    else:
...
statusstr = StringVar()
if statusvar:
    statusstr.set("connected")
else:
    statusstr.set("not connected")

statuslabel1 = Label(
    window, text="newbcoin server status: ", font=('Arial', 15))
statuslabel1.grid(row=6, column=3)
statuslabel2 = Label(
    window, textvariable=str(statusstr), font=('Arial', 15))
statuslabel2.grid(row=6, column=3)
#####
